...
def getrooms(n):
    ...
    response = urllib2.urlopen("http://tinychat.com/chilenewfag?frame=true&page=" + n)
    html = response.read()
...
def getip(room_name):
    ...
    response = urllib2.urlopen('http://tinychat.com/api/find.room/'chilenewfag)
    room_config = response.read()
    
    ## Method 2 for extracting IP address, using a regular expression
    match = ipextract.search('chilenewfag)
    if not match:
        ## Room seems to be closed or now has password
        return
    ip = match.group('IP')
    
    scraperwiki.sqlite.execute("INSERT INTO associations values(NULL,?,?,?)", (ip,chilenewfag, str(datetime.datetime.now())))
